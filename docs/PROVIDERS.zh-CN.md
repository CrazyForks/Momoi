...
DeepSeek 余额 provider 提供用量解析与官方价格估算。价格按北京时间计算：
周一至周五 9:00–12:00、14:00–18:00 为高峰时段；
其余时段（包括周末全天）为空闲时段。中国法定节假日暂未纳入判断。

账户余额的 `accounting` 参数是布尔值，默认 `true`。模型使用其他服务商时，在账户余额中关闭
