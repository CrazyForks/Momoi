...
class StorageMemoryTest(unittest.TestCase):
    ...
    def test_owner_commit_preserves_last_heartbeat_activity(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            store = Store(Path(directory) / "momoi.sqlite3")
            store._db.execute(
                """UPDATE self_state SET activity='reading', activity_result='finished',
            ...
            self.assertEqual(
                tuple(state[key] for key in ("activity", "activity_result", "activity_since",
                                            "last_heartbeat_at", "next_heartbeat_at")),
                ("reading", "finished", 500, 600, 2000),
            )
            store.close()
